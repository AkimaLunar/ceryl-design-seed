// =============================================================================
// Plane
// =============================================================================

export const PLANE_SHADOW_0 =
    '0 0.1rem 0.2rem rgba(0,0,0,0.1), 0 0.1rem 0.6rem rgba(0,0,0,0.1)'
export const PLANE_SHADOW_2 =
    '0 0.1rem 0.2rem rgba(0,0,0,0.1), 0 0.4rem 1rem rgba(0,0,0,0.15)'
export const PLANE_SHADOW_4 =
    '0 0.1rem 0.2rem rgba(0,0,0,0.1), 0 0.6rem 1.2rem rgba(0,0,0,0.175)'
export const PLANE_SHADOW_6 =
    '0 0.2rem 0.2rem rgba(0,0,0,0.1), 0 0.9rem 1.6rem rgba(0,0,0,0.2)'
export const PLANE_SHADOW = PLANE_SHADOW_2
